...
def validate_mingguan_json(result):
    """Memastikan struktur JSON memiliki kunci yang tepat sebelum disetujui."""
    if not isinstance(result, dict):
        raise ValueError("Output JSON bukan sebuah dictionary.")
    
    bm = result.get("bonus_mingguan", [])
    if not isinstance(bm, list):
        raise ValueError("Kunci 'bonus_mingguan' harus berupa list.")
    
    for period in bm:
        if "tiers" not in period:
            raise ValueError("Setiap period di 'bonus_mingguan' harus memiliki kunci 'tiers'.")
        for tier in period["tiers"]:
            if not all(k in tier for k in ("jumlah_pesanan", "biasa", "berganda")):
                raise ValueError("Tiers di 'bonus_mingguan' ada yang kurang (jumlah_pesanan/biasa/berganda).")

    bh = result.get("bonus_harian", {})
    if bh:
        if not all(k in bh for k in ("date_range", "jumlah_pesanan", "bonus")):
            raise ValueError("Kunci 'bonus_harian' ada yang kurang (date_range/jumlah_pesanan/bonus).")
...
def run_gemini_mingguan_fallback(image_path, api_key, log=st.write):
    ...
    for model_name in model_fallback_list:
        ...
        try:
            ...
            last_err = e
            log(f"⚠️ [{nama_model_pendek}] gagal validasi/ekstraksi ({e}), coba model berikutnya...")
            continue
    raise Exception(f"Fallback Mingguan gagal di semua model. Error terakhir: {last_err}")
...
def validate_puncak_json(result):
    ...
    if not isinstance(result, list):
        raise ValueError("Output JSON Tambang Puncak harus berupa list array.")
    for item in result:
        if not isinstance(item, dict):
             raise ValueError("Isi list Tambang Puncak harus berupa dictionary/object.")
        if not all(k in item for k in ("Date", "Day", "Hour", "Bonus")):
            raise ValueError("Kunci item Tambang Puncak ada yang kurang (Date/Day/Hour/Bonus).")
...
def extract_peak_fare(image_path, api_key, year=TAHUN_SEKARANG, log=st.write):
    ...
    for model_name in model_fallback_list:
        ...
        try:
            ...
            log(f"⚠️ [{nama_model_pendek}] gagal validasi/ekstraksi ({e}), coba model berikutnya...")
            continue
    raise Exception(f"Ekstraksi Puncak gagal di semua model. Error terakhir: {last_err}")
