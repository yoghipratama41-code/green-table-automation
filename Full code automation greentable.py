...
# ==========================================
st.set_page_config(page_title="GNS Greentable Automator", page_icon="🟢", layout="wide")
...
# Gems Automator pakai spreadsheet yang BEDA dari greentable (sesuai app.py aslinya)
...
# ==========================================
# 1. KONEKSI SPREADSHEET (SERVICE ACCOUNT, sama kayak app.py — tanpa login manual)
# ==========================================
@st.cache_resource
def get_spreadsheet():
...
def upload_to_gsheets(df, tab_name):
    ...
    worksheet.clear()
    # USER_ENTERED supaya tanggal & angka tidak jadi format text dengan kutip (')
...
@st.cache_resource
def get_gems_sheet():
    """Koneksi terpisah untuk spreadsheet Gems Automator (beda dari greentable)."""
    scopes = ["https://www.googleapis.com/auth/spreadsheets"]
    creds = Credentials.from_service_account_info(
        dict(st.secrets["gcp_service_account"]), scopes=scopes
    )
...
# ==========================================
# 2. CACHE ENGINE OCR (sama pattern kayak app.py)
# ==========================================
...
def _call_with_retry(model, contents, nama_model_pendek, step_label, log, max_retry=2):
    """Satu model, maksimal `max_retry` percobaan, HANYA retry kalau kena rate limit (429/503)."""
    delay = 5
    last_err = None
...
def run_gemini_mingguan_fallback(image_path, api_key, log=st.write):
    """Coba model satu-per-satu (urut prioritas), pakai model PERTAMA yang berhasil."""
...
# Aturan target cells (kembaran):
#   1) Dalam 1 tier yang sama: E-bike = Bicycle
#   2) Diamond & Sapphire: vehicle yang sama saling terhubung, KECUALI Walker
#   3) Ruby & Emerald: tidak ada hubungan lintas-tier
...
def trigger_color_compare(row_numbers):
    """Kirim daftar nomor baris yang baru diupdate ke Apps Script Web App untuk pewarnaan."""
...
# 6. UI UTAMA
# ==========================================
st.title("🟢 GNS Greentable Automator")
st.caption("Upload gambar insentif mingguan (full image) → diekstrak otomatis → masuk ke Google Sheets.")

try:
    sh = get_spreadsheet()
    st.success(f"✅ Terhubung ke spreadsheet: **{sh.title}**")
except Exception as e:
...
gemini_api_key = st.sidebar.text_input("Gemini API Key (fallback jika OCR gagal / untuk Tambang Puncak & Gems)", type="password")
st.sidebar.caption(f"Target tab greentable:\n- `{TAB_MINGGUAN}`\n- `{TAB_PUNCAK}`")

st.divider()

uploaded_image = st.file_uploader(
    "Unggah gambar insentif mingguan (full image, bukan crop)",
    type=["jpg", "jpeg", "png"],
)
...
if uploaded_image is not None:
    ...
    st.image(image_path, caption="Preview gambar", width=350)
...
# (spreadsheet TERPISAH dari greentable, upload gambar batch tersendiri — sesuai app.py asli)
# ------------------------------------------
with tab_gems:
    st.subheader("💎 Automasi Ekstraksi Gems & Insentif")
    st.caption("Upload *screenshot* mission (bisa banyak sekaligus) → diekstrak EasyOCR/Gemini → masuk ke Spreadsheet Gems.")
    ...
    st.info(f"📌 **Target Utama:** {selected_tier} - {selected_vehicle}\n\n"
            f"🔄 **Auto-fill ke:** {', '.join(target_display)}")

    gems_files = st.file_uploader(
        "Unggah Gambar Mission (bisa sekalian banyak/batch)",
        type=["jpg", "jpeg", "png"],
        accept_multiple_files=True,
        key="gems_uploader",
    )

    if gems_files and st.button("🚀 Jalankan Ekstraksi", type="primary", key="run_gems"):
        ...
        st.balloons()
        st.success("🎉 Seluruh gambar selesai diproses dan dimasukkan ke Spreadsheet Gems!")
